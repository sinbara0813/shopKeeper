...
public class PurchaseWarehouseSplitAdapterAdapter extends DelegateAdapter.Adapter<AllotOrderListHolder> {
    ...
    public PurchaseWarehouseSplitAdapterAdapter(Context context, RequestManager manager, List<PurchaseItemListBean.RecordsBean> orderList) {
    ...
    private void setOrderData(AllotOrderListHolder holder, int position) {
        ...
        PurchaseItemListBean.RecordsBean purchaseItemBean = mOrderList.get(position);
        ...
        tvProductSpec.setText(purchaseItemBean.getStandard());
        tvProductName.setText(purchaseItemBean.getProductName());
        tvProductPrice.setText(mContext.getString(R.string.label_money, Util.getNumberFormat(purchaseItemBean.getSupplyPrice())));
        tvProductStock.setText(mContext.getString(R.string.label_quantity, String.valueOf(purchaseItemBean.getQuantity())));
        if (!StringUtil.isEmpty(purchaseItemBean.getProductPic())) {
            ImageLoader.displayImage(manager, getValidPic(purchaseItemBean.getProductPic().split(",")), ivProduct, R.mipmap.ic_logo_empty5, R.mipmap.ic_logo_empty5);
        } else {
        ...
        if(purchaseItemBean.getDifference()>=1){
        ...
        holder.tvOrderStatus.setText(purchaseItemBean.getStatusName());
        holder.tvAction.setText(mContext.getString(R.string.label_order_code, purchaseItemBean.getPurchSn()));
        holder.tvOrderDetial.setText(mContext.getString(R.string.label_product_total, String.valueOf(purchaseItemBean.getQuantity()), Util.getNumberFormat(purchaseItemBean.getTotalAmount())));
    ...
    private void setButtons(AllotOrderListHolder holder, int position) {
                    ...
                    new AlertDialog.Builder(mContext)
                            ...
                            .setPositiveButton("确认", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    cancelPurchaseDiff(mOrderList.get(position).getId());
                                }
                            }).show();
                }
            });
        ...
        if ("DELIVER".equals(mOrderList.get(position).getStatus())) {
            ...
            textView1.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    WarehousingConfirmPop warehousingConfirmPop = new WarehousingConfirmPop(mContext, 1, mOrderList.get(position).getId(), mOrderList.get(position).getStandard(), mOrderList.get(position).getQuantity());
    ...
    private void cancelPurchaseDiff(long id) {
        ...
        ((BaseActivity)mContext).addDisposable(ApiRetrofit.getInstance().getApiService().purchCancelDiff(arrayMap), new BaseObserver() {
            @Override
