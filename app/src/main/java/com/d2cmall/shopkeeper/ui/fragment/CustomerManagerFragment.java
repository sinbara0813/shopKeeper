...
import com.d2cmall.shopkeeper.adapter.CustomerManagerAdapter;
 ...
 * Description : CustomerManagerFragment
 * 客户管理
 */

public class CustomerManagerFragment extends BaseFragment implements BaseView {
    ...
    private List<CustomerListBean.RecordsBean> customerList;
    private UserBean user;
    private boolean needEnd;
    private BaseVirtualAdapter<DefaultHolder> endAdapter; //列表结束标志

    public static CustomerManagerFragment newInstance(int type) {
        CustomerManagerFragment buyerSaleFragment = new CustomerManagerFragment();
    ...
    public void initView() {
        ...
        customerManagerAdapter = new CustomerManagerAdapter(getActivity(), Glide.with(mContext), customerList,type);
        delegateAdapter.addAdapter(customerManagerAdapter);
    ...
    private void loadCustomerList() {
        //type=0 :消费客户  =1关注客户
        if(type==0){
            addDisposable(ApiRetrofit.getInstance().getApiService().getCustomerList(Util.buildListParam(p, 20)), new BaseObserver<BaseModel<CustomerListBean>>(this) {

                @Override
                public void onSuccess(BaseModel<CustomerListBean> customerBean) {
                    setDataToView(customerBean);
                }
            ...
            addDisposable(ApiRetrofit.getInstance().getApiService().getCustomerFocusList(Util.buildListParam(p, 20)), new BaseObserver<BaseModel<CustomerListBean>>(this) {
    ...
    private void setDataToView(BaseModel<CustomerListBean> customerBean) {
        ptr.refreshComplete();
        if (p == 1) {
            customerList.clear();
        }
        if (customerBean != null && customerBean.getData() != null && customerBean.getData().getRecords() != null && customerBean.getData().getRecords().size() > 0) {
            ...
            customerList.addAll(customerBean.getData().getRecords());
        } else {
            if (p == 1) {
