...
public class AllotProductFragment extends HeaderViewPagerFragment implements BaseView{
    ...
    public static AllotProductFragment newInstance(int type) {
        AllotProductFragment buyerSaleFragment = new AllotProductFragment();
    ...
    private void loadData(){
        addDisposable(ApiRetrofit.getInstance().getApiService().getAllotItemList(buildParam(p, 20)), new BaseObserver<BaseModel<AllotItemListBean>>() {
            @Override
            public void onSuccess(BaseModel<AllotItemListBean> allotListBean) {
                if (p==1){
                    list.clear();
                }
                if (allotListBean.getData()!=null&&allotListBean.getData().getRecords()!=null&&allotListBean.getData().getRecords().size()>0){
                    hasNext=allotListBean.getData().getCurrent()<allotListBean.getData().getPages()?true:false;
                    list.addAll(allotListBean.getData().getRecords());
                }else {
                    if (p==1){
    ...
    public void prepareView() {
        ...
        allotPurchaseAdapter = new AllotPurchaseAdapter(getActivity(),getManager(),list,0);
